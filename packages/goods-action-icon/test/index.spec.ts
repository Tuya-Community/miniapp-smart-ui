...
describe('goods-action-icon', () => {
  const SmartGoodsActionIcon = simulate.load(
    path.resolve(__dirname, '../index'),
    'smart-goods-action-icon',
    {
  ...
  test('should render with default props', async () => {
    const comp = simulate.render(
      simulate.load({
        usingComponents: {
          'smart-goods-action-icon': SmartGoodsActionIcon,
        },
        template: `<smart-goods-action-icon id="wrapper" />`,
      })
    );
    ...
    expect(wrapper?.data.classPrefix).toBe('smart-icon');
  });
