...
```json
"usingComponents": {
  "smart-calendar": "@tuya-miniapp/smart-ui/calendar/index"
}
```
...
```html
<smart-cell title="选择单个日期" value="{{ date }}" bind:click="onDisplay" />
<smart-calendar show="{{ show }}" bind:close="onClose" bind:confirm="onConfirm" />
```
...
```html
<smart-cell title="选择多个日期" value="{{ text }}" bind:click="onDisplay" />
<smart-calendar
  show="{{ show }}"
  type="multiple"
  bind:close="onClose"
...
```html
<smart-cell title="选择日期区间" value="{{ date }}" bind:click="onDisplay" />
<smart-calendar
  show="{{ show }}"
  type="range"
  bind:close="onClose"
...
```html
<smart-calendar show="{{ show }}" show-confirm="{{ false }}" />
```
...
```html
<smart-calendar show="{{ show }}" color="#07c160" />
```
...
```html
<smart-calendar show="{{ show }}" type="range" formatter="{{ formatter }}" />
```
...
```html
<smart-calendar type="range" max-range="{{ 3 }}" />
```
...
> Tips: 注意，在自定义calendar的高度时，需要确保，滚动到当前月份时，所有的日期要展现在.smart-calendar__body内，否则可能会出现滚动时头部月份与当前月份不同步的情况。目前平铺型预设高度618px,弹窗型高度90%。
