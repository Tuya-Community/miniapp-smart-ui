...
The `mode` property represents the type, angle, and angle2 are semicircular types. The `angle-offset`  `v2.8.0` property is used to set the starting angle offset for semicircular types (`angle`, `angle2`), in degrees. The larger the value, the larger the gap in the circle. The default value is -1, which means using the default offset.
...
| -------------------------- | ------------ | ------------- | -------------------------- |
| angle-offset  `v2.8.0`  | Angle Offset | number        | -1                         |
