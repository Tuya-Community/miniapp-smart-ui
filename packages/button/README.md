...
```json
"usingComponents": {
  "smart-button": "@tuya-miniapp/smart-ui/button/index"
}
```
...
| --- | --- | --- | --- |
| id | 标识符 | _string_ | - |
| button-id | 标识符，作为原生button组件的id值 | _string_ | - |
| type | 按钮类型，可选值为 `primary` `info` `warning` `danger` | _string_ | `default` |
| size | 按钮尺寸，可选值为 `normal` `large` `small` `mini` | _string_ | `normal` |
| color | 按钮颜色，支持传入`linear-gradient`渐变色 | _string_ | - |
| icon | 左侧图标或图片链接，可选值见 [Icon 组件](#/icon) | _string_ | - |
| right-icon | 右侧图标或图片链接，可选值见 [Icon 组件](#/icon) | _string_ | - |
| class-prefix | 图标类名前缀，同 Icon 组件的 [class-prefix 属性](#/icon) | _string_ | `smart-icon` |
