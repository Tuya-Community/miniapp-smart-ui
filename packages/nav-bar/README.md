...
```json
"usingComponents": {
  "smart-nav-bar": "@tuya-miniapp/smart-ui/nav-bar/index"
}
```
...
Page({
  ...
  onClickRight() {
    ty.showToast({ title: '点击按钮', icon: 'none' });
  },
});
```
...
```html
<smart-nav-bar title="标题" left-text="返回" left-arrow>
  <smart-icon name="search" slot="right" />
</smart-nav-bar>
```
